...
调度器中以状态机的设计模型来控制流水线中各个子概念的状态流转。
首先以作业为例介绍作业状态机的详细设计。
...
调度器的执行逻辑在图 3.14 的时序图中有详细展示。系统初始化时，Scheduler类的processTask方法在守护线程中运行，不断地处理作业。当API服务接收到用户发起的流水线事件时，会通过addJob方法将作业提交至消息队列中进行排队。调度器作为一个分布式系统，任何节点都有能力处理这些作业。为了确保作业不丢失，在进行决策前，必须通过lockJob方法对作业加锁，以获得处理权。

Scheduler在接收到一批待处理作业后，会根据每个作业的具体事件类型及其包含的状态进行详细决策。完成决策后，决策结果Decision将被检查，如果包含对作业的决策，则将其发送回作业管理器以便进一步处理，这可能会导致作业状态的变更，此类状态变更可由状态机处理。若决策中未包含对整个作业的处理，而是针对特定作业的，则会触发作业管理器的相关处理逻辑，并通过后端服务进行状态持久化。
